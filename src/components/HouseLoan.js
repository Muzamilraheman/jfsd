...
export default function Dashboard() {
  ...
  const { id } = useParams();
  const [amountpaid, setPaidAmount] = useState("");
  ...
  const loadUser = async () => {
    const result = await axios.get(`http://localhost:8080/homeloan/${id}`);
    setUser(result.data);
    console.log(result.data)
  };
  const loadPaidAmount = async () => {
    
      const response = await axios.get(`http://localhost:8080/paidloans/${id}`);
      setPaidAmount(response.data.paidamount);
      console.log(response.data.paidamount);
          ...
          <Container maxWidth="lg" sx={{ mt: 4, mb: 4 }}>
            ...
            <div>
              ...
              <Grid item xs={12}>
                ...
                <React.Fragment>
      <Link to={`/applyhouseloan`}><Title>Apply Home Loan</Title></Link>
      <Link to={`/payhomeloan`}><Title>Pay Loan</Title></Link>
